...
const Groups = () => {
  ...
  // === Create group (validate and POST) ===
  ...
  const resetModal = () => { setSubGroups([]); setCreateError(null); setShowModal(false); };
                  ...
                  <div className="modal-body">
                    ...
                    {subGroups.map((sub, index) => (
                      <div key={index} className="mb-4 border p-3 rounded">
                        ...
                        <div className="mb-2">
                          <label className="form-label">Description *</label>
                          <textarea className="form-control" rows="2" required value={sub.description} onChange={(e) => setSubGroups(prev => prev.map((sg,i) => i===index?{...sg, description: e.target.value}:sg))}></textarea>
                        </div>
                      </div>
                    ))}
                  </div>
